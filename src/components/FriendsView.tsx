...
export const FriendsView: React.FC<FriendsViewProps> = ({ onBack, onJoinInvite }) => {
    ...
    <div className="min-h-screen bg-game-bg px-4 py-6 sm:px-6">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900">好友</h1>
            <p className="text-sm font-medium text-slate-400">添加好友、处理申请、查看房间邀请</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[32px] p-5 card-shadow space-y-4">
          ...
          <div className="flex gap-3">
            <input
              ...
              placeholder="输入对方手机号"
              className="flex-1 px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-primary/20"
            />
            <button
              onClick={async () => {
                if (!phone) return;
              ...
              className="px-5 py-3 bg-primary text-white rounded-2xl font-black shadow-lg shadow-primary/20"
            >
              添加
            </button>
          </div>
